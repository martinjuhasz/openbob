...
function makeChannel(name: string, jidPrefix: string, connected = true): Channel {
  return {
    name,
...
describe('formatOutbound', () => {
  it('strips internal tags and returns text', () => {
    expect(formatOutbound('hello <internal>x</internal> world')).toBe('hello  world')
  })
...
describe('checkTrigger', () => {
  it('detects trigger word', () => {
    expect(checkTrigger([makeMessage('@yetaclaw hello')], 'yetaclaw')).toBe(true)
  })

  it('detects trigger without @', () => {
    expect(checkTrigger([makeMessage('yetaclaw please do this')], 'yetaclaw')).toBe(true)
  })
...
describe('routeOutbound', () => {
  ...
  it('throws when no channel found', () => {
    const ch = makeChannel('mm', 'mm:')
    expect(() => routeOutbound([ch], 'tg:abc', 'hi')).toThrow('No channel for JID')
  })

  it('throws when channel disconnected', () => {
    const ch = makeChannel('mm', 'mm:', false)
    expect(() => routeOutbound([ch], 'mm:abc', 'hi')).toThrow('No channel for JID')
  })
})
