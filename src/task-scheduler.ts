...
export function computeNextRun(task: ScheduledTask): number | null {
  ...
  if (task.scheduleType === 'cron') {
    try {
      ...
      logger.warn({ taskId: task.id, value: task.scheduleValue }, 'Invalid cron expression')
...
async function runTask(task: ScheduledTask, deps: SchedulerDeps): Promise<void> {
  ...
  const sessionId =
    task.contextMode === 'group' ? (deps.getSession(task.groupFolder) ?? undefined) : undefined

  try {
    const model = process.env['MODEL'] ?? 'anthropic/claude-sonnet-4-5'
    const [providerID = 'anthropic', ...modelParts] = model.split('/')
    const modelID = modelParts.join('/') || providerID
    ...
    if (output.status === 'success' && output.result) {
      ...
      logger.error({ taskId: task.id, error: output.error }, 'Task agent error')
    }
  } catch (err) {
