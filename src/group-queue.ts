...
export class GroupQueue {
  ...
  constructor(maxConcurrent?: number) {
    this.maxConcurrent =
      maxConcurrent ??
      parseInt(process.env['MAX_CONCURRENT_AGENTS'] ?? '5', 10);
  }
  ...
  private async runForGroup(
    ...
    try {
      ...
      // eslint-disable-next-line no-catch-all/no-catch-all -- queue resilience: schedule retry
    } catch (err) {
  ...
  private async runTask(groupJid: string, task: QueuedTask): Promise<void> {
    ...
    try {
      await task.fn();
      // eslint-disable-next-line no-catch-all/no-catch-all -- queue resilience: isolate task failure
    } catch (err) {
  ...
  private scheduleRetry(groupJid: string, state: GroupState): void {
    ...
    if (state.retryCount > MAX_RETRIES) {
      logger.error(
        ...
        'Max retries exceeded, resetting',
      );
      state.retryCount = 0;
      return;
    }
