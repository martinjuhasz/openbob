...
export class MattermostChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    logger.info({ userId: me.id, username: me.username }, 'Mattermost bot authenticated')
  ...
  private async handleEvent(event: WebSocketEvent): Promise<void> {
    ...
    try {
      const user = await this.api<{ username: string; first_name: string; last_name: string }>(
...
registerChannel('mattermost', (opts: ChannelOpts) => {
  ...
  if (!url || !token) {
    logger.warn('MATTERMOST_URL or MATTERMOST_TOKEN not set — skipping Mattermost channel')
    return null
  }
